...
# Load audio into correct format for ASR model
    for files in segm_wav:
        ...
        formatted_audio.append((waveform.squeeze().numpy(), 16000))
...
def main():
    ...
    print(f"Number of items returned by model.transcribe: {len(results_list)}")
    predictions = []
    for rec_word in results:
        predictions.append(rec_word.text)
