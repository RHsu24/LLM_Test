...
def format_audio(segm_wav):
    ...
    return audio_frames, audio_files
...
def main():
    ...
    audio_frames, audio_files = format_audio(segm_wav)


    ####### MODEL SPECIFIC CODE ############
    model = SALM.from_pretrained('nvidia/canary-qwen-2.5b').bfloat16().eval().to(device)
    # Have to convert to tensors in-situ for SALM

    BATCH_SIZE = 16
    predictions = []
    for batch_idx, batch_audios in enumerate(stream_audio_batches(audio_files, BATCH_SIZE)):
        tensor_list = [torch.as_tensor(audio).float().squeeze() for audio in batch_audios] # Convert arrays into 1D PyTorch tensors
